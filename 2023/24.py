...
def main(file: str) -> None:
    ...
    hailstones = u.input_as_lines(file, map=lambda l: tuple(u.find_digits(l)))

    p1 = 0
    for i, (x1, y1, _, dx1, dy1, _) in enumerate(hailstones):
        for (x2, y2, _, dx2, dy2, _) in hailstones[i + 1:]:
            x, y = line_line_intersection(
                x1, y1, x1 + dx1, y1 + dy1, x2, y2, x2 + dx2, y2 + dy2)
            if x is not None and y is not None and MIN <= x <= MAX and MIN <= y <= MAX and (x - x1) / dx1 >= 0 and (x - x2) / dx2 >= 0:
                p1 += 1
    print(f'{p1=}')
