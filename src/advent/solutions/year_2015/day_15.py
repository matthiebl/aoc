...
class Day15(Solver):
    ...
    def part2(self) -> int:
        ...
        for teaspoons in self.nums_that_sum_to(self.TEASPOONS, length=len(self.store.ingredients)):
            ...
            if any(p <= 0 for p in properties[:-1]):
                continue
            if properties[-1] == self.CALORIE_TARGET:
                score = reduce(lambda x, y: x * y, properties[:-1])
                highest_score = max(highest_score, score)
        return highest_score

    def nums_that_sum_to(self, target: int, length: int = 2) -> Generator[tuple]:
        ...
        for i in range(1, target - length + 1):
            for tail in self.nums_that_sum_to(target - i, length - 1):
                yield (i,) + tail
